...
function draw(){
  ...
  // if(ball_x-rec_x>0&&ball_x-rec_x<rec_w&&ball_y>=height-(ball_dia/2+rec_h)){
  //   dy=(-dy);
  // }
  ...
  // if(width-ball_y<rec_h&&rec_x+rec_w<ball_x-(ball_dia/2)&&dx<0){
  //   dx=-(dx);
  // }
  ...
  time++;
   ...
   fill('red');
      textSize(16);
      textAlign(RIGHT);
      text(Math.round(time/60)+' sec',200,20);
      textAlign(LEFT);
      textSize(10)
  // game over
  if(life<=0){
...
// feature
// velocity vary acording to plank direction
// speed twice at score 5
// make score in 3 min
// bricks regenraate when ball collide with plank if time left (3 min)
// bricks fall after time out
// messege at 3 range of score timeout, nice try, well done
//
